...
class ModelExtensionTotalWebposTotal extends Model {
	public function getTotal($total) {
		...
		if (isset($this->session->data['instalment'])){
			...
			$price=$total['total'];
			$ratio=floatval($this->config->get('payment_webpostotal_single_ratio'));
			if($instalment!=0){
				$webpos_total=($price*$bank_array[2])/100;
				$webpos_title=$this->language->get('text_total').'('.$instalment.'x'.$this->currency->format((float)(($price+$webpos_total)/$instalment),'TRY').')';
			} else {
				if ($ratio>0){
